<html>
    <head>
        ...
        $fechaHoy = date('Y-m-d');
        ?>
        <link rel="stylesheet" href="estiloestCompras.css">
    </head>
    <body>
        <div class="container">
            ...
            if(!empty($_POST['fechaD']) && !empty($_POST['fechaH']) && $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                if($fechaDesde <= $fechaHasta) {
                    ...
                    $ordenarProd = mysqli_query($link,
                        "SELECT producto.nombre, SUM(producto_compra.cantidad) as total_cantidad 
                         FROM producto 
                         JOIN producto_compra ON producto.id = producto_compra.idProd 
                         JOIN compra ON compra.id = producto_compra.idCompra 
                         WHERE fecha_compra BETWEEN '$fechaDesde' AND '$fechaHasta'
                         GROUP BY producto.id, producto.nombre 
                         ORDER BY total_cantidad DESC");
                    
                    $resultTotal = mysqli_query($link,
                        "SELECT SUM(producto_compra.cantidad) as total
                         FROM producto_compra 
                         JOIN compra ON producto_compra.idCompra = compra.id 
                         WHERE fecha_compra BETWEEN '$fechaDesde' AND '$fechaHasta'");
                    ...
                    if(mysqli_num_rows($ordenarProd) > 0) {
                        ...
                        echo '<p style="color:orange;">No se encontraron compras en ese período.</p>';
                    }
            ...
            <?php if($hayDatos): ?>
                <div class="resultados-container">
                    ...
                    <div style='background:#e3f2fd; padding:15px; margin:10px 0; border-radius:8px;'>
                        <h4>Total de unidades compradas: <strong><?php echo $totalUnidades; ?></strong></h4>
                    </div>
                    
                    <!-- ⭐ GRÁFICO PRIMERO -->
                    <div class="grafico-container">
                        <h3>Gráfico de Productos Comprados</h3>
                ...
                <script type="text/javascript">
                    ...
                    function drawChart() {
                        ...
                        var options = {
                            title: 'Productos Más Comprados (Total: <?php echo $totalUnidades; ?> unidades)',
                            titleTextStyle: {
                                fontSize: 18,
                                bold: true,
                                color: '#2c3e50'
